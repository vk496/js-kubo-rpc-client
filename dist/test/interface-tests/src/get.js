    ...
    describe('.get', function () {
        ...
        describe('files', function () {
            it('should get with a base58 encoded multihash', async () => {
                const output = await pipe(ipfs.get(fixtures.smallFile.cid), tarballed, collect);
            ...
            it('should get a file added as CIDv0 with a CIDv1', async () => {
                ...
                const output = await pipe(ipfs.get(cidv1), tarballed, collect);
                expect(output).to.have.lengthOf(1);
            ...
            it('should get a file added as CIDv1 with a CIDv0', async () => {
                ...
                const output = await pipe(ipfs.get(cidv0), tarballed, collect);
                expect(output).to.have.lengthOf(1);
            ...
            it('should get a BIG file', async () => {
                const output = await pipe(ipfs.get(fixtures.bigFile.cid), tarballed, collect);
                expect(output).to.have.lengthOf(1);
            ...
            it('should get with ipfs path, as object and nested value', async () => {
                ...
                const output = await pipe(ipfs.get(`/ipfs/${fileAdded.cid}/testfile.txt`), tarballed, async (source) => collect(source));
            ...
            it('should compress a file directly', async () => {
                const output = await pipe(ipfs.get(fixtures.smallFile.cid, {
                    compress: true,
                    compressionLevel: 5
                }), gzipped, async (source) => collect(source));
            ...
            it('get path containing "+"s', async () => {
                ...
                const output = await pipe(ipfs.get(CID.parse(cid)), tarballed, collect);
        ...
        describe('directories', function () {
            it('should get a directory', async function () {
                ...
                const output = await pipe(ipfs.get(cid), tarballed, async (source) => collect(source));
                // Check paths
            ...
            it('should compress a directory as a tarball', async () => {
                ...
                const { cid } = res[res.length - 1];
                const output = await pipe(ipfs.get(cid, {
                    archive: true,
                    compress: true,
                    compressionLevel: 5
                }), gzipped, tarballed, async (source) => collect(source));
                // Check paths
                const paths = output.map((file) => { return file.header.name; });
                expect(paths).to.include.members([
